...
**Date:** 2026-07-16
**Tickets:** `backend/tickets/P10_TICKETS.md`

### Built
- **1 migration:** `tickets` (uuid, guest_id? FK, `chatbot_session_id` — plain nullable column, **not** an FK yet since `chatbot_sessions` doesn't exist until P11, conversation_id? FK→conversations, subject, category, status default `open`, priority int default 2, department, `source` default `chatbot` — named in ARCHITECTURE §5.11 prose but absent from §7's abbreviated schema, added now to avoid a P11 remediation migration, assigned_user_id? FK→users).
- **Model:** `Ticket` — status/category/source constants, `CATEGORY_DEPARTMENTS` map (a new judgment-call mapping — PLAN specifies none; `complaint`→concierge, `maintenance`→housekeeping, `booking_help`→reception, `inquiry`/`other`→concierge, same default-to-concierge fallback shape as `ServiceRequest::TYPE_DEPARTMENTS`).
- **3 Actions:** `RouteRequestAction` (shared department resolver, `ServiceRequest|Ticket` — no HTTP route in P10, same treatment as P4's `ReleaseExpiredHoldsAction`; nothing creates a `Ticket` until P11, so it's unit-tested directly and left for `CreateTicketAction` to call), `AssignRequestAction`, `UpdateRequestStatusAction` (both mirror to the same `ops_queue` Firestore collection P9's `MirrorServiceRequestToFirestore` already writes to, via a new shared `App\Support\OperationsQueueMirror` payload builder).
- **Service:** `OperationsQueueService` — `index()` (true merge of both tables, each side included only if the caller holds its own `.view` permission, bounded to 500 rows/table since a cross-table merge can't be paginated at the DB layer, sorted newest-first, hand-paginated), `summary()` (permission-gated aggregate counts), `assign()`/`updateStatus()` (resolve-by-`{type}` + in-service permission check, since the two tables need different permissions per operation that static route middleware can't branch on).
- **Resource:** `OperationsQueueItemResource` — normalizes `ServiceRequest`/`Ticket` into one shape (`type` discriminator + shared fields).
- **Routes (`auth:users`):** `GET /operations/queue` (`service_requests.view|tickets.view`), `PATCH /operations/queue/{type}/{uuid}/assign`, `PATCH /operations/queue/{type}/{uuid}/status`, `GET /dashboard/summary`.
- **Factory:** `TicketFactory`.
- **Docs:** `API_GUIDE_DASHBOARD.md` — new "Operations Queue & Dashboard" module.
- **6 new test files** (`tests/Feature/Operations/`).

### Deviations / Decisions (recorded in `P10_TICKETS.md` before coding, reproduced here)
1. **Unified queue scope = `service_requests` + `tickets` only**, per PLAN's own wording ("no new core tables except tickets; this unifies queues", "unified queue merges requests + tickets"). ARCHITECTURE §5.7's aspect table lists a broader read scope (Events, Pre-Arrival, In-room, Tickets) — treated as descriptive context, not a scope expansion, consistent with how P7 already resolved the same ARCHITECTURE-vs-PLAN tension. `GET /dashboard/summary`, being read-only aggregate counts with no new mutation surface, does go slightly wider — it adds an `event_inquiries` block gated by the same `tickets.view` permission P6 already used for event-inquiry admin routes.
2. **`RouteRequestAction` has no real caller in P10** — nothing in this phase creates a `Ticket`, and P6/P7's existing inline routing (`SubmitInquiryAction`, `PlaceServiceRequestAction`) works and is tested; refactoring shipped code to delegate to the new shared primitive for DRY's sake alone was judged out of scope. Verified via direct unit-style Action calls against both models — P11's `CreateTicketAction` is its intended caller.
3. **Assign/status permission checked in the service, not route middleware.** `service-requests` vs `tickets` need different permissions per operation (`service_requests.assign` vs `tickets.assign`; `service_requests.update` vs `tickets.respond`); Laravel's static `permission:` middleware can't branch on a URL segment, so `OperationsQueueService::assertCan()` resolves the required permission from `{type}` + operation and throws `ForbiddenException` directly.

### Bug found during testing (test-authoring pitfall, not a product bug)
Two of the new tests combined a "wrong permission → 403" and "right permission → 200" assertion for two *different* users in one test method (two separate `patchJson` calls). Both failed with an unexpected 403 on the second call. Root cause: Laravel's Auth guard caches the resolved user for the lifetime of the test's `$this->app` instance, which is **not** reset between two HTTP calls within a single test method (only a full test-to-test boundary rebuilds it) — the exact same class of issue P1's `StaffLoginTest` already worked around with an explicit `auth()->forgetGuards()`. Rather than sprinkle that workaround here, split each combined test into two separate test methods (`..._is_forbidden` / `..._succeeds`) — matches this codebase's existing one-assertion-focus-per-test convention, which is why the pitfall hadn't surfaced anywhere else yet.

### Naive Reviewer Result: PASS WITH WARNINGS (8 fixed, 1 reviewed and left as-is)
Workflow-backed `/code-review high` run against the full diff — 4 finders, independent verify pass, 12 candidates collapsing to 9 distinct findings.
1. **Fixed — unguarded synchronous Firestore mirror calls could 500 an already-successful write.** `SendMessageAction`, `AssignRequestAction`, `UpdateRequestStatusAction` (and P9's `MirrorServiceRequestToFirestore`) all call `mirrorToFirestore()` with no error handling, after their DB write already committed — a Firestore failure would surface as a 500 on a request that actually succeeded, and a client retry on that 500 could duplicate the underlying write (e.g. a second chat message). Fixed at the root: `MirrorsToFirestore` trait now catches and logs (`Log::warning`) instead of letting the exception propagate — every current and future consumer gets this for free. Added `FirestoreMirrorResilienceTest`.
2. **Fixed — `priority` silently changed JSON type per row in the merged queue.** `ServiceRequest.priority` is a string enum (`low`/`normal`/`high`); `Ticket.priority` is a 1–3 int. Both flowed unchanged into `OperationsQueueItemResource` and the Firestore mirror payload, so the same field was a string for one row and an int for the next in the same paginated array — a typed client would throw on one or the other. Added `Ticket::priorityLabel()` (normalizes the int scale to the same low/normal/high vocabulary) and used it in both the Resource and `OperationsQueueMirror::payload()`. Added `test_queue_priority_is_a_consistent_type_across_both_item_types`.
3. **Fixed — the new global `Relation::morphMap()` entries collided with Spatie ActivityLog.** Adding `'guest' => Guest::class, 'staff' => User::class` for `Message::sender` is process-wide, and both models are also polymorphic `causer`/`subject` targets for ActivityLog (via `LogsActivity`, used by nearly every model) — every future audit-log row for a `User`/`Guest` actor would silently start being written as `'staff'`/`'guest'` instead of the FQCN, breaking any query filtering by class name against older rows. Removed both entries from the morph map; `Message.sender_type` now stores the FQCN directly (default Eloquent behavior), and a new `Message::senderLabel()` maps it back to `"guest"`/`"staff"` for API output — the external contract in `API_GUIDE_MOBILE.md`/tests is unchanged.
4. **Fixed — device-token unique-constraint race between two different guests.** P9's fix locked the *guest* row to serialize same-guest concurrent registrations, but two *different* guests racing on the exact same never-before-seen token could still both pass the (per-guest) lock and collide on the token's DB-level unique constraint, throwing an unhandled `QueryException`. `RegisterDeviceTokenAction` now catches that specific violation and takes ownership of the row the other request just won, instead of 500ing.
5. **Fixed — the merge-fetch cap silently hid older *active* items once a table passed 500 rows.** Rather than just raising the number, `OperationsQueueService::index()` now filters each table to non-terminal statuses first (`new`/`in_progress` for service requests, `open`/`assigned` for tickets) before applying the 500-row cap — a queue should show unresolved work, not full history, so the realistic active-row count is now what the cap actually bounds. Added `test_queue_excludes_completed_and_resolved_items`.
6. **Fixed — stale doc comment.** `NotificationService::notifyDepartment()` claimed department notifications were "queryable later by the dashboard (P10)" — P10's `OperationsQueueService` doesn't read `guest_notifications` at all. Corrected the comment to say so plainly rather than leave a misleading claim for a future maintainer.
7. **Fixed — redundant `DB::transaction()` around a single `update()` call** in both `AssignRequestAction` and `UpdateRequestStatusAction` (no atomicity benefit, one write statement each). Removed.
8. **Fixed — `MirrorServiceRequestToFirestore` (P9) built its own inline Firestore payload instead of reusing the new `OperationsQueueMirror` builder**, leaving two independently-maintained shapes for the same `service_request_*` document that could drift apart. `OperationsQueueMirror::payload()` extended to include `guest_uuid`/`created_at` (matching what the listener already sent); the listener now calls it directly.
9. **Reviewed, no change — `UpdateRequestStatusRequest` validates against `ServiceRequest::STATUSES` for an unrecognized `{type}`, not just `tickets`/`service-requests`.** True, but harmless: `OperationsQueueService::assertCan()` still rejects any unrecognized `{type}` with a 404 before the resource is ever resolved (`test_unknown_type_segment_returns_404` already covers this). Tightening the validation rule to reject unknown types at the 422 layer would actually make the response *less* correct — a 404 ("no such queue type") is the right code for a bad URL segment, and forcing a 422 instead would perversely replace a correct response with an incorrect one for a defense-in-depth gain that doesn't otherwise change behavior.

### Bug found during testing (test-authoring pitfall, not a product bug)
Two of the new tests combined a "wrong permission → 403" and "right permission → 200" assertion for two *different* users in one test method (two separate `patchJson` calls). Both failed with an unexpected 403 on the second call. Root cause: Laravel's Auth guard caches the resolved user for the lifetime of the test's `$this->app` instance, which is **not** reset between two HTTP calls within a single test method (only a full test-to-test boundary rebuilds it) — the exact same class of issue P1's `StaffLoginTest` already worked around with an explicit `auth()->forgetGuards()`. Rather than sprinkle that workaround here, split each combined test into two separate test methods (`..._is_forbidden` / `..._succeeds`) — matches this codebase's existing one-assertion-focus-per-test convention, which is why the pitfall hadn't surfaced anywhere else yet.

### Stop-and-Report
- **Tests:** 250 passed, 0 failed, 0 skipped (226 prior + 24 new P10)
- **Assertions:** 655
- **New suites:** `tests/Feature/Operations/RouteRequestActionTest.php` (4), `OperationsQueueTest.php` (14), `DashboardSummaryTest.php` (4), `tests/Feature/Notification/FirestoreMirrorResilienceTest.php` (2)
- `migrate:fresh --seed` verified clean from empty DB with the new `tickets` migration; P0–P9 suites unchanged
- **New error_codes introduced:** none (unknown `{type}` reuses `not_found`; permission failures reuse `forbidden`; bad status value reuses `validation_failed`)

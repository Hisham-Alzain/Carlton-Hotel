...
class BookingSeeder extends Seeder
{
    ...
    private function ratePlansAndPricing(): void
    {
        ...
        foreach (RoomType::where('name->en', 'Deluxe King')->orWhere('name->en', 'Executive Suite')->get() as $roomType) {
            PricingRule::create([
                'room_type_id' => $roomType->id, 'scope' => PricingScope::SEASONAL,
                'starts_on' => now()->startOfMonth()->toDateString(), 'ends_on' => now()->addMonths(2)->toDateString(),
                'modifier_type' => ModifierType::PERCENTAGE, 'modifier_value' => 20, 'is_active' => true,
            ]);
            PricingRule::create([
                'room_type_id' => $roomType->id, 'scope' => PricingScope::WEEKEND,
                'starts_on' => now()->startOfMonth()->toDateString(), 'ends_on' => now()->addMonths(6)->toDateString(),
                'modifier_type' => ModifierType::PERCENTAGE, 'modifier_value' => 15, 'is_active' => true,
            ]);
        }
    }
    ...
    {
        PromoCode::create(['code' => self::PROMO_ACTIVE, 'type' => ModifierType::PERCENTAGE, 'value' => 10, 'expires_at' => now()->addMonths(2), 'max_uses' => null, 'used_count' => 0, 'is_active' => true]);
        PromoCode::create(['code' => self::PROMO_WELCOME, 'type' => ModifierType::PERCENTAGE, 'value' => 20, 'expires_at' => now()->addMonths(6), 'max_uses' => 100, 'used_count' => 3, 'is_active' => true]);
        // Deliberately expired — for testing the invalid_promo error path.
        PromoCode::create(['code' => self::PROMO_EXPIRED, 'type' => ModifierType::FLAT, 'value' => 5, 'expires_at' => now()->subMonth(), 'max_uses' => null, 'used_count' => 0, 'is_active' => true]);
    }
    ...
    private function reservations(): void
    {
        ...
        $laylaReservation = Reservation::create([
            'guest_id' => $layla->id, 'booking_code' => 'CARL-DEMO0002', 'source' => ReservationSource::DIRECT,
            'check_in' => now()->addDays(5)->toDateString(), 'check_out' => now()->addDays(8)->toDateString(),
            'status' => ReservationStatus::CONFIRMED, 'payment_method' => PaymentMethod::ON_ARRIVAL, 'total_usd' => 840.00,
        ]);
        ...
        $saraReservation = Reservation::create([
            'guest_id' => $sara->id, 'booking_code' => 'CARL-DEMO0003', 'source' => ReservationSource::DIRECT,
            'check_in' => now()->subDays(10)->toDateString(), 'check_out' => now()->subDays(7)->toDateString(),
            'status' => ReservationStatus::CHECKED_OUT, 'payment_method' => PaymentMethod::CASH, 'total_usd' => 270.00,
        ]);
        ...
        $omarReservation = Reservation::create([
            'guest_id' => $omar->id, 'booking_code' => 'CARL-DEMO0004', 'source' => ReservationSource::DIRECT,
            'check_in' => now()->addDays(3)->toDateString(), 'check_out' => now()->addDays(4)->toDateString(),
            'status' => ReservationStatus::CANCELLED, 'payment_method' => PaymentMethod::ON_ARRIVAL, 'total_usd' => 90.00,
        ]);
        ...
        $raniaReservation = Reservation::create([
            'guest_id' => $rania->id, 'booking_code' => 'CARL-DEMO0005', 'source' => ReservationSource::DIRECT,
            ...
            'payment_method' => PaymentMethod::ON_ARRIVAL, 'total_usd' => 180.00,
        ]);
        ...
        foreach (Guest::whereNotIn('phone', [
            ...
            $reservation = Reservation::create([
                ...
                'source' => ReservationSource::DIRECT, 'check_in' => $checkIn->toDateString(),
                'check_out' => $checkIn->copy()->addDays($nights)->toDateString(),
                'status' => ReservationStatus::PENDING, 'payment_method' => PaymentMethod::ON_ARRIVAL,

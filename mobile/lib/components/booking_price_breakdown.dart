...
/// `GET /public/quote` result the caller holds (the API returns no separate tax
/// line — the total already nets the discount). Add-ons are UI-only (no backend
/// concept) so they no longer appear in the charged breakdown.
class BookingPriceBreakdown extends StatelessWidget {
  final BookingPriceSummary summary;

  const BookingPriceBreakdown({required this.summary, super.key});
